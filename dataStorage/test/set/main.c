 ...
 * 	set with invalid order of param (fail)
 */

START_TEST (test_setinvalid_invalidtable)
{
	struct storage_record record;
	strncpy(record.value, "brand Nissan,price 34102", sizeof record.value);
	int status = storage_set(BADTABLE, KEY1, &record, test_conn);
	fail_unless(status == -1, "storage_set with invalid table should fail.");
	fail_unless(errno == ERR_INVALID_PARAM, "storage_set with invalid table not setting errno properly.");
}
END_TEST


START_TEST (test_setinvalid_order)
{
	struct storage_record record;
	strncpy(record.value, "grade 78,id 999123812", sizeof record.value);
	int status = storage_set(STUDENTSTABLE, KEY4, &record, test_conn);
	fail_unless(status == -1, "storage_set with invalid order should fail.");
	fail_unless(errno == ERR_INVALID_PARAM, "storage_set with invalid order not setting errno properly.");
}
END_TEST
...
START_TEST (test_set_deletemissing)
{
	// Delete a key/value pair.
	int status = storage_set(SUBWAYTABLE, KEY4, NULL, test_conn);
...
START_TEST (test_setsample_deletestudents)
{
	// Delete a key/value pair.
	int status = storage_set(STUDENTSTABLE, KEY1, NULL, test_conn);
	fail_unless(status == 0, "Error deleting the key/value pair.");

	// Try to get the deleted value.
	struct storage_record record;
	strncpy(record.value, "", sizeof record.value);
	status = storage_get(STUDENTSTABLE, KEY1, &record, test_conn);
...
START_TEST (test_setsample_updatecars) {
	struct storage_record record;
	int fields = 0;
	int intval = 0;
	...
	// Update the value
	strncpy(record.value, "brand BMW,price 92122", sizeof record.value);
	int status = storage_set(CARSTABLE, KEY1, &record, test_conn);
	fail_unless(status == 0, "Error updating a value.");

	// Get the new value.
	strncpy(record.value, "", sizeof record.value);
	status = storage_get(CARSTABLE, KEY1, &record, test_conn);
	fail_unless(status == 0, "Error getting a value.");
	fields = sscanf(record.value, "brand %[a-zA-Z0-9] , price %d", strval, &intval);
	fail_unless(fields == 2, "Got wrong number of fields.");
	fail_unless(intval == 92122, "Got wrong value.");
	fail_unless(strcmp(trimtrailingspc(strval), "BMW") == 0, "Got wrong value.");
}
END_TEST
...
int main(int argc, char *argv[])
{
    ...
    // Set test with invalid order
    tc = tcase_create("setinvalidorder");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_sample_populate, test_teardown);
	tcase_add_test(tc, test_setinvalid_order);
	suite_add_tcase(s, tc);
    
    // Set test with invalid table
    tc = tcase_create("setinvalidtable");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_sample_populate, test_teardown);
	tcase_add_test(tc, test_setinvalid_invalidtable);
	suite_add_tcase(s, tc);
    
    // Set test with updating record
    tc = tcase_create("setupdaterecord");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_sample_populate, test_teardown);
	tcase_add_test(tc, test_setsample_updatecars);
	suite_add_tcase(s, tc);
    
    // Set test with deleting record
    tc = tcase_create("setdeleterecord");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_sample_populate, test_teardown);
	tcase_add_test(tc, test_setsample_deletestudents);
	suite_add_tcase(s, tc);
    
    // Set test with deleting missing record
    tc = tcase_create("setdeletemissing");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_sample_populate, test_teardown);
	tcase_add_test(tc, test_set_deletemissing);
	suite_add_tcase(s, tc);
